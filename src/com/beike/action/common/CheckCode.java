...
public class CheckCode extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		String chacternumber[] = new String[] { "A", "B", "C", "D", "E", "F",
		...
		for (int i = 0; i < 4; i++) {
			...
			if (i % 2 == 0) {
				rand = chacternumber[random.nextInt(chacternumber.length)];
			}
			sRand += rand;
			// 将认证码显示到图象中
		...
		memCacheService.set("validCode_" + codeuuid, sRand);

		// logger.info(session.getId());
		// logger.info("存入session中的验证码是："+session.getAttribute("validCode"));
