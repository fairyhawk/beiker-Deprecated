...
import com.beike.util.XiaoneiUtils;
 ...
 */
@Controller
public class XiaoneiWeiboAction {
	private final Log log = LogFactory.getLog(XiaoneiWeiboAction.class);
	...
	private static String RENREN="RENREN";
	...
	@RequestMapping("/user/xiaoneiBindAccount.do")
	public Object bindWeiboAccount(ModelMap model, HttpServletRequest request,
			HttpServletResponse response) {
		// 假如没有被绑定过
		XiaoNeiAccessToken xAccessToken = null;
//		RenrenSessionKey rsk = null;
		request.setAttribute("WEIBO_SITE_TYPE",
				propertyUtil.getProperty("RENREN_APP_NAME"));
		String xuuid = WebUtils.getCookieValue("XWEIBOUUID", request);

		xAccessToken = (XiaoNeiAccessToken) memCacheService
				.get("XIAONEI_ACCESSTOKEN_" + xuuid);
		...
		String weiboName = xAccessToken.getScreenName();
		...
		request.setAttribute("WEIBO_USER_SCREENNAME",
				xAccessToken.getScreenName());
		request.setAttribute("WEIBO_URL", "/user/xiaoneiBindAccount.do");
		if (bindType == null) {
		...
		weiboService = (WeiboService) BeanUtils.getBean(request,
				"XIAONEICONFIGService");
		if (user != null) {

			Long userid = user.getId();
			Long bindUserid = weiboService
					.getBindingAccessTokenByWeiboId(xAccessToken.getRenren_id()
							+ "", ProfileType.XIAONEICONFIG);
			...
			if (bindUserid != 0) {
				...
				if (!((bindUserid + "").equals(userid + ""))) {
					weiboService.removeBindingAccessTokenByWeiboId(
							xAccessToken.getRenren_id() + "",
							ProfileType.XIAONEICONFIG);
					XiaoNeiAccessToken bAccessToken = (XiaoNeiAccessToken) weiboService
							.getBindingAccessToken(userid,
									ProfileType.XIAONEICONFIG);
					if (bAccessToken.getRenren_id() != 0) {
						weiboService.updateBindingAccessToken(xAccessToken,
								userid, ProfileType.XIAONEICONFIG);
					} else {
						weiboService.addBindingAccess(userid, xAccessToken,
								ProfileType.XIAONEICONFIG);
					}
				}
			} else {
				XiaoNeiAccessToken bAccessToken = (XiaoNeiAccessToken) weiboService
						.getBindingAccessToken(userid,
								ProfileType.XIAONEICONFIG);
				if (bAccessToken.getRenren_id() != 0) {
					weiboService.updateBindingAccessToken(xAccessToken, userid,
							ProfileType.XIAONEICONFIG);
				} else {
					weiboService.addBindingAccess(userid, xAccessToken,
							ProfileType.XIAONEICONFIG);
			...
			if ("CREATEACCOUNT".equals(bindType)) {
				...
				// 新用户绑定
				weiboService.addBindingAccess(newUser.getId(), xAccessToken,
						ProfileType.XIAONEICONFIG);
				...
				try {
					...
					if (userProfile == null) {
						userService
								.addProfile(Constant.EMAIL_REGIST_URLKEY,
										secret, newUser.getId(),
					...
					// 设置动态参数
					Object[] emailParams = new Object[] { time, sb.toString(),
							date };
				...
				// 先判断之前这个微博账户 是否绑定过其他账户 假如绑定了先清除 最后给这个账户绑定上 update by ye.tian
				// at 2011.6.19
				...
				if (bindUserid != 0) {
					...
					if (!((bindUserid + "").equals(newUser.getId() + ""))) {
						...
						weiboService.addBindingAccess(newUser.getId(),
								xAccessToken, ProfileType.XIAONEICONFIG);
					}
				} else {
					weiboService.addBindingAccess(newUser.getId(),
							xAccessToken, ProfileType.XIAONEICONFIG);
	...
	// @RequestMapping("/user/getXiaoneiUserinfo.do")
	// public String getSinaUserinfo(ModelMap model,HttpServletRequest request){
	//
	// RenrenSessionKey rsk=(RenrenSessionKey)
	// request.getSession().getAttribute("XIAONEI_REQUESTTOKEN");
	// XiaoNeiAccessToken xAccessToken = (XiaoNeiAccessToken)
	// request.getSession().getAttribute("XIAONEI_ACCESSTOKEN");
	// if(rsk==null){
	// request.setAttribute("ERRMSG", "USER_TIMEOUT");
	// return "user/useraccount";
	// }
	//
	// XiaoneiUser xiaoneiUser = XiaoneiUtils.getUserInfo(rsk.getSessionkey(),
	// rsk
	// .getRenrenid()
	// + "");
	//
	// User user=(User) request.getSession().getAttribute(Constant.USER_LOGIN);
	// if(user!=null){
	// XiaoNeiAccessToken aAccessToken=(XiaoNeiAccessToken)
	// weiboService.getBindingAccessToken(user.getId(), ProfileType.SINACONFIG);
	// int renrenUserid=aAccessToken.getRenren_id();
	// //此用户是否和此微博绑定 绑定了就不用管 没绑定更新
	// aAccessToken.setScreenName(xiaoneiUser.getName());
	// if(renrenUserid!=0){
	// //用户绑定过 但是不是用的这个账号
	// if(renrenUserid!=user.getId()){
	//
	// weiboService.updateBindingAccessToken(xAccessToken, user
	// .getId(), ProfileType.XIAONEICONFIG);
	// }
	// }else{
	// weiboService.addBindingAccess(user.getId(), xAccessToken,
	// ProfileType.XIAONEICONFIG);
	// }
	// }
	//
	...
	@RequestMapping("/user/xiaoneiAuthorization.do")
	public Object redirectOAuthzPage(ModelMap model,
		...
		// 获得回调地址
		String callbackUrl = propertyUtil
				.getProperty("XIAONEICONFIG_REDIRECT_URL");
		XiaoNeiAccessToken xAccessToken = (XiaoNeiAccessToken) XiaoneiUtils
				.getAccessToken(authorization_code, callbackUrl);
		if(xAccessToken == null){
			request.setAttribute("ERRMSG", "授权错误,请重新授权!");
			return new ModelAndView("user/login");
		}
		...
		if (trxauthz != null && "trxauthz".equals(trxauthz)) {
			...
			request.getSession().setAttribute("trxauthz", trxauthz);
		}
		RenrenSessionKey rsk = XiaoneiUtils
				.getXiaonei_SessionKey_info(xAccessToken.getAccess_token());
		...
		XiaoneiUser xiaoneiUser = XiaoneiUtils.getUserInfo(rsk.getSessionkey(),
				rsk.getRenrenid() + "");
		// 人人id 
		xAccessToken.setRenren_id(rsk.getRenrenid());
		xAccessToken.setScreenName(xiaoneiUser.getName());
		xAccessToken.setHeadIcon(xiaoneiUser.getHeadurl());
		
		String uuidstr = UUID.randomUUID().toString();
		int validy = 60 * 60 * 1;
		Cookie cookie = WebUtils.cookie("XWEIBOUUID", uuidstr, validy);
		...
		// memCacheService.set("WEIBO_USER_SCREENNAME_"+uuidstr,
		// xiaoneiUser.getName());
		request.setAttribute("WEIBO_USER_SCREENNAME", xiaoneiUser.getName());
		memCacheService.set("XIAONEI_ACCESSTOKEN_" + uuidstr, xAccessToken);
		// memCacheService.set("XIAONEI_REQUESTTOKEN_"+uuidstr, rsk);
		// request.getSession().setAttribute("WEIBO_USER_SCREENNAME",
		// xiaoneiUser.getName());
		// request.getSession().setAttribute("XIAONEI_ACCESSTOKEN",
		// xAccessToken);
		// request.getSession().setAttribute("XIAONEI_REQUESTTOKEN", rsk);
		weiboService = (WeiboService) BeanUtils.getBean(request,
				"XIAONEICONFIGService");
		User user = SingletonLoginUtils.getMemcacheUser(request);
		if (user != null) {
			...
			// XiaoNeiAccessToken aAccessToken=(XiaoNeiAccessToken)
			// weiboService.getBindingAccessToken(user.getId(),
			// ProfileType.SINACONFIG);
			Long bindUserid = weiboService.getBindingAccessTokenByWeiboId(
					rsk.getRenrenid() + "", ProfileType.XIAONEICONFIG);
			// int renrenUserid=aAccessToken.getRenren_id();
			Long userid = user.getId();
			// 此用户是否和此微博绑定 绑定了就不用管 没绑定更新
			if (bindUserid != 0) {
			...
			Map<String, String> weiboNames = weiboService.getWeiboNames(user
					.getId());
			...
			// request.getSession().setAttribute("WEIBO_NAMES", weiboNames);
			request.setAttribute("WEIBO_NAME", xiaoneiUser.getName());
			// return "user/useraccount";
		} else {
			request.setAttribute("WEIBO_URL", "/user/xiaoneiBindAccount.do");
