...
/**
 * 
 * @author zhaojingllong
 *
 */
public class WapBaiduApiThread extends Thread {
	private final Log logger = LogFactory.getLog(WapBaiduApiThread.class);
	...
	public WapBaiduApiThread(String trxid, String baiduUserId, String chkGoodsId, 
				...
				if(lstGoodsCount!=null && lstGoodsCount.size()>0){
					for(int i=0;i<lstGoodsCount.size();i++){
						...
						if(goodsId.equals(chkGoodsId)){
							...
							baiduParams.setUrl("http://m.qianpin.com/wap/goods/goodsDetailController.do?method=queryDetailShowMes&goodsIds=" + goodsId);
